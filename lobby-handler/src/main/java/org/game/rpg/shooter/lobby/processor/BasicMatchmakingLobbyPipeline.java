...
@Slf4j
@AllArgsConstructor
public class BasicMatchmakingLobbyPipeline implements StreamPipeline {

  final ReadyLobbyAggregator readyLobbyAggregator;
  final BasicMatchmakingLobbyPipelineProperties basicPipelineProperties;

  @Override
  public Properties initProperties() {
    Properties properties = new Properties();
    properties.put(StreamsConfig.APPLICATION_ID_CONFIG, basicPipelineProperties.getApplicationId());
    properties.put(
        StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, basicPipelineProperties.getBootstrapServer());
    return properties;
  }
  ...
  public Topology buildTopology() {
    ...
    KStream<SearchMatchmaking, MatchmakingLobby> gameLobbyEventStream =
        streamsBuilder.stream(
                basicPipelineProperties.getInputTopic(),
                Consumed.with(
    ...
    gameLobbyEventStream
        ...
        .to(basicPipelineProperties.getOutputTopic());
  ...
  private static void logLobbyCreationEvent(MatchmakingLobby v) {
    log.info("-------------");
    log.info(
        "L0BBY HAS BEEN CREATED : {} FOR : mode=[{}/{}], ranked=[{}/{}]",
        v.getLobbyID(),
        v.getMode(),
